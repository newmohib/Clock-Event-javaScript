...
var decreaseStartClock = function (result) {
    ...
    var elementName = result.elementName;
    var key = result.key;
    if (result.isStart == true) {
        setToStore(useStorage, "decreaseSetTime", result);
        decreaseClock();
    } else {
        stopClock(useStorage,"decreaseSetTime", elementName)
    }
}


var increaseStartClock = function (result) {
    let elementName = result.elementName;
    let key = result.key;
    if (result.isStart == true) {
        setToStore(useStorage,"increaseSetTime", result);
        increaseClock();
    } else {
        stopClock(useStorage, "increaseSetTime", elementName)
    }
...
var continueAfterPauseClock = function (result) {
    ...
    let getTime = getFromStore(useStorage, key);
    if (getTime) {
        getTime.elementName = result.elementName;;
